...
입력 받는 서브넷 마스크에 대해 값이 올바르게 들어오는지에 대한 확인이 필요하다는 요구사항이 들어와 제작하게 됨
...
로직 형태
	...
	2. 자른 값들을 비교하여 [n]보다 [n+1]이 더 작을 경우 에러로 처리

	3. 서브넷 마스크에 들어올 수 있는 값(알맞은 값)이 한정적임으로 배열에서 비교하여 해당 값이 아닐경우 에러로 처리
